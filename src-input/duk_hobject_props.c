...
DUK_LOCAL duk_uint32_t duk__push_tval_to_property_key(duk_context *ctx, duk_tval *tv, duk_hstring **out_h) {
	...
	/* XXX: value stack policy is awkward: we need to push because the
	 * result may be a coerced duk_hstring not referenced from anywhere
	 * else.  But if property key was in a value stack slot, we could
	 * just overwrite that slot.
	 */
	...
	if (DUK_TVAL_IS_STRING(tv)) {
		...
		DUK_TVAL_SET_STRING(tv_dst, h);  /* no decref needed */
...
DUK_INTERNAL duk_bool_t duk_hobject_delprop(duk_hthread *thr, duk_tval *tv_obj, duk_tval *tv_key, duk_bool_t throw_flag) {
	...
	if (DUK_TVAL_IS_OBJECT(tv_obj)) {
		...
		arr_idx = duk__push_tval_to_property_key(ctx, tv_key, &key);
		DUK_ASSERT(key != NULL);
		duk_replace(ctx, -2);  /* FIXME: awkward helper */
...
DUK_INTERNAL duk_ret_t duk_hobject_object_get_own_property_descriptor(duk_context *ctx) {
	...
	DUK_ASSERT(obj != NULL);
	key = duk_to_hstring_acceptsymbol(ctx, 1);
	DUK_ASSERT(key != NULL);
